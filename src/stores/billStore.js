  ...
  actions: {
    ...
    async fetchBills() {
      ...
      try {
        ...
        const response = await axios.get(`https://${this.baseUrl}/api/bills`, {
    ...
    async addBill() {
      try {
        const token = localStorage.getItem('token')
        const response = await axios.post(`https://${this.baseUrl}/api/bills`, this.newBill, {
    ...
    async fetchBillSummary() {
      ...
      try {
        const token = localStorage.getItem('token')
        const response = await axios.get(`https://${this.baseUrl}/api/bills/summary`, {
          headers: { Authorization: `Bearer ${token}` },
        })
    ...
    async fetchUnpaidBills() {
      ...
      try {
        const token = localStorage.getItem('token')
        const response = await axios.get(`https://${this.baseUrl}/api/bills/unpaid`, {
          headers: { Authorization: `Bearer ${token}` },
        })
    ...
    async updateBillStatus(billId, newStatus) {
      try {
        const token = localStorage.getItem('token')
        await axios.put(
          `https://${this.baseUrl}/api/bills/${billId}`,
          { status: newStatus },
          { headers: { Authorization: `Bearer ${token}` } },
        )
    ...
    async fetchOverdueBills() {
      try {
        const token = localStorage.getItem('token')
        const response = await axios.get(`https://${this.baseUrl}/api/bills/overdue`, {
          headers: { Authorization: `Bearer ${token}` },
        })
